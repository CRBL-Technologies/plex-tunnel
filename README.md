...
The client exposes a local status page at `http://127.0.0.1:9090/` showing connection state, session ID, pool size, active websocket count, and recent errors. Settings can be changed from the UI without restarting the container. Authentication is now form-based: set `PLEXTUNNEL_UI_PASSWORD` to require login, optionally set `PLEXTUNNEL_UI_USERNAME` to require both username and password, and use `PLEXTUNNEL_UI_SESSION_TTL` to control how long the session cookie lasts. This is a breaking change from prior versions that used HTTP Basic Auth, so any scripts that were sending Basic Auth to the UI must be updated.
